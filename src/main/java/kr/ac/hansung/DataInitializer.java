...
if (productRepository.count() == 0) {
    ...
    productRepository.save(new Product("JPA 프로그래밍 실전", 32000,
    ...
    productRepository.save(new Product("애플 맥북 프로 M4", 2990000,
            "M4 칩 탑재 고성능 노트북", 0));
    ...
    productRepository.save(new Product("삼성전자 65인치 QLED TV", 1590000,
            "Neo QLED 4K 스마트 TV", 5));

    productRepository.save(new Product("다이슨 V15 청소기", 890000,
            "레이저 먼지 감지 무선 청소기", 18));

    productRepository.save(new Product("닌텐도 스위치 2", 459000,
            "휴대·거치 겸용 게임 콘솔", 0));

    productRepository.save(new Product("애플 에어팟 프로 3", 359000,
            "공간 음향 지원 무선 이어폰", 55));
}
